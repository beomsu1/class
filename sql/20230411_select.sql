...
-- 사원테이블에서 사원의 이름과 급여 , 연봉계산 결과를 출력 연봉은 급여*12
...
-- , _ 한자리 __ 두자리 -> __A 앞 두자리는 어떤 문자가와도 상관 X 뒤에 a로 끝나는 두자리 문자열


--F로 시작하는 이름의 사원 리스트
select * from emp where ename like 'F%';

-- 이름에 A가 포함하는 사원 리스트 '%A%' , '%JAVA%'
...
-- select ... from ... where .. order by 컬럼이름 asc , desc . asc는 생략가능

-- 급여순으로 사원 리스트 출력 오름차순
select * from emp order by sal asc;

-- 급여순으로 사원 리스트 출력 내림차순
