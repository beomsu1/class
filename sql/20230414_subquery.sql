...
select custid, sum(saleprice) , ( SELECT name from customer c where c. custid = o. custid) as name,
...
-- any : 서브쿼리의 결과 데이터 비교해서 하나 이상 참 -> 참
select *
from orders
where saleprice > all (select saleprice from orders where custid=3)
;
-- saleprice > 6000 and saleprice > 12000 and saleprice > 13000 다 만족하는값 
...
-- exists , no exists
-- 상관관계가 반드시 필요한 서브쿼리가 조건의 결과로 사용 : select 결과가 존재하는 여부에 따라 
